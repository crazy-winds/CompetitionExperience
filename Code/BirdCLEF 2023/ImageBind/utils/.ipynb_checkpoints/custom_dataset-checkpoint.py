...
class CustomDatset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.sample = upsample_data(self.df, 600, seed)
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            time_step = random.random() * (self.duration - 4) + 4
    ...
    def mixup(self, orig_x, label, num_sample=1):
        ...
        for _ in range(num_sample):
            idx = random.randint(0, len(self.sample) - 1)
            name, path, time = self.sample.loc[idx]
            label = self.cls2id[name]
            ...
            x, orig_sr = librosa.load(
                ...
                sr=None,
                offset=random.randint(0, max(time - 5, 0.)),
                duration=random.random() * (self.duration - 4) + 4,
            )
            if orig_sr != self.SR:
                x = librosa.resample(x, orig_sr=orig_sr, target_sr=self.SR)
